...
class Rest {
    ...
    func getStringFromHTTPRequest(urlString: String, completion: @escaping (Result<String, Error>) -> Void) {
    ...
    func getAnswerFromHTTPRequest(urlString: String, completion: @escaping (Result<String, Error>) -> Void) {
        guard let url = URL(string: urlString) else {
            let error = NSError(domain: "Invalid URL", code: 0, userInfo: nil)
            completion(.failure(error))
            return
